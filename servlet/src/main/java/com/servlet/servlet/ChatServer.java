...
public class ChatServer {
    ...
    public void onMessage(String message, Session session) throws IOException {
        ...
        try {
            ...
            List<Long> userIdsOfChat = MessagesService.getUserIdsOfChat(chatId);

            for (Session local : sessions) {
                Long localId = (Long) local.getUserProperties().get("userId");
                if(userIdsOfChat.contains(localId)) {
                    local.getBasicRemote()
                            .sendText(new ObjectMapper().writeValueAsString(msg));
                }
            }
